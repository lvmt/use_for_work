    ...
    def get_snv_info(self):
        ...
        with utils.safe_open(self.vcf, 'r') as fr:
            for line in fr:
                ...
                gt = linelist[head_index['case']].split(':')[0]
                if case:
                    ...
                    case_var_freq = round((case_alt / case_pos_depth) * 100, 2)
                    vcf_info[key].update({
        ...
        with utils.safe_open(self.vcf, 'r') as fr:
            for line in fr:
                ...
                if 'cancer' in head_index:
                    ...
                    case_info = linelist[head_index['cancer']].split(':')
                    gt = case_info[0]
                    case_ref, case_alt = case_info[1].split(',')
                    case_pos_depth = int(case_info[-1])
                    case_var_freq = round((int(case_alt) / int(case_pos_depth) * 100), 2)
                    vcf_info[key].update({
                        'Case_ref_readsNum': case_ref,
                        'Case_var_readsNum': case_alt,
                ...
                if 'normal' in head_index:
                    ...
                    control_info = linelist[head_index['normal']].split(':')
                    control_ref, control_alt = control_info[1].split(',')
                    control_pos_depth = int(control_info[-1])
                    vcf_info[key].update({
                        'Ctrl_ref_readsNum': control_ref,
                        'Ctrl_var_readsNum': control_alt,
