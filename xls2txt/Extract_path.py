...
class ExtractPath(object):
    ...
    def extract_target_path(self):
                ...
                if self.sample_info_dict:
                    ...
                    if novoid in self.sample_info_dict:
                        if samid == self.sample_info_dict[novoid][0]:
                            ...
                            warinng_message[novoid] = [samid,self.sample_info_dict[novoid][0]]
                            standername = 'Wrong'
                else:
                    standername = samid
        ...
        if self.sampleinfo:
            ...
            if warning_message:
                print('\033[1;44m有下机单和信息搜集表中名称不一致的样本如下：\033[0m')
                for novoid in warning_message:
                    print(novoid)
                    print '下机单名称：',
                    print(warning_message[novoid][0])
                    print '信息搜集表名称: ',
                    print(warning_message[novoid][1].decode('utf-8'))
            else:
